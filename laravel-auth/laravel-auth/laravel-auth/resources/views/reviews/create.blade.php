<x-app-layout>
    ...
    <div class="max-w-4xl mx-auto py-8">
        <h1 class="text-2xl font-bold text-gray-700 mb-6">Add Review</h1>
        <form action="{{ route('reviews.store') }}" method="POST" class="space-y-6 bg-white shadow-md rounded-lg p-6">
            @csrf
            <div>
            ...
            <div>
                ...
                <input type="number" name="rating" id="rating" class="block w-full mt-2 p-2 border border-gray-300 rounded-md" min="1" max="5" required>
            </div>

            <div>
                <label for="komentar" class="block text-gray-700 font-medium">Komentar</label>
                <textarea name="komentar" id="komentar" class="block w-full mt-2 p-2 border border-gray-300 rounded-md" rows="4" required></textarea>
            </div>
